...
smells'')}{Problem indicators (bad smells)}}\label{modelling-bad-smells}
...
\midrule
\endhead
No ``recommended'' components as smell?\tabularnewline
\bottomrule
\end{longtable}

(Bad) Smells are used as simple indicators for bad or inefficient
designs (typically in programming languages). To identify a specific
``smell'' in a profile does not necessarily mean that the design is
broken and should be replaced. Instead it should be seen as a warning
sign that there may be a problem which should be checked. Measures to
eliminate actual problems by changing the design are often called
refactorings. In the following some ``smells'' and refactorings (adapted
from their counterpart in object oriented programming) are shortly
discussed:
...
Should we rather move these general remarks (``(Bad) Smells are \ldots{}
often called refactorings'') to the ``How to use this Guide'' section?
\strut\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
